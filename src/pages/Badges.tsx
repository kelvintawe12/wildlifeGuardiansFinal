      ...
      <div className="md:col-span-2">
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {myBadges.map(badge => {
              // Robust fallback for all possible badge property sources
              const b = badge.badges || badge.badge || badge;
              return (
                <div key={badge.id || b.id} className="bg-gradient-to-b from-white to-gray-50 rounded-lg p-4 border border-gray-100 flex flex-col items-center text-center hover:shadow-md transition-shadow">
                  <div className="w-20 h-20 rounded-full overflow-hidden mb-3 border-2 border-yellow-200">
                    <img src={badge.image_url || b.image_url} alt={badge.name || b.name} className="w-full h-full object-cover" />
                  </div>
                  <h3 className="font-bold text-gray-800 mb-1">{badge.name || b.name}</h3>
                  <span className={`text-xs px-2 py-1 rounded-full mb-2 ${badge.category === 'Quiz' || b.category === 'Quiz' ? 'bg-green-100 text-green-800' : badge.category === 'Achievement' || b.category === 'Achievement' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}`}>
